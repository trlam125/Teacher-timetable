  ...
  document.querySelectorAll(".visual-captcha").forEach((captcha) => {
    ...
    if (kind === "images") {
      ...
      tiles.forEach((tile) => {
        tile.addEventListener("click", () => {
          ...
          tile.setAttribute(
            "aria-pressed",
            tile.classList.contains("is-selected") ? "true" : "false",
          );
          update();
        });
      });
    ...
    if (kind === "puzzle") {
      ...
      const update = () => {
        ...
        answer.value = pieces
          .map((piece) => piece.dataset.captchaValue)
          .join(",");
      ...
      board.querySelectorAll(".captcha-puzzle-piece").forEach((piece) => {
        piece.addEventListener("click", () => {
          if (!active) {
            ...
            status.textContent =
              "Đã chọn 1 mảnh. Chọn mảnh thứ hai để đổi chỗ.";
            return;
          }
        ...
        piece.addEventListener("dragstart", (event) => {
          ...
          if (event.dataTransfer) {
            ...
            event.dataTransfer.setData(
              "text/plain",
              piece.dataset.captchaValue || "piece",
            );
          }
        });
    ...
    form.addEventListener("submit", (event) => {
      ...
      if (kind === "images") {
        const selectedCount = answer.value
          ? answer.value.split(",").filter(Boolean).length
          : 0;
