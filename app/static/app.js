...
.assignment-load-card{display:grid;grid-template-columns:repeat(2,minmax(150px,1fr));gap:8px;margin:10px 0 12px;padding:10px;border:1px solid var(--line);border-radius:12px;background:var(--bg)}
.assignment-load-metric{min-width:0;padding:8px 10px;border-radius:9px;background:var(--card-bg);border:1px solid var(--line)}
.assignment-load-metric small{display:block;margin-bottom:3px;color:var(--muted);font-size:11px;font-weight:700}.assignment-load-metric b{display:block;color:var(--ink);font-size:15px;white-space:nowrap}.assignment-load-metric.over{border-color:#fca5a5;background:#fef2f2}.assignment-load-metric.over b{color:#b91c1c}
:root.dark-mode .assignment-load-metric.over{border-color:#ef4444;background:rgba(239,68,68,.15)}:root.dark-mode .assignment-load-metric.over b{color:#fca5a5}
.assignment-load-cell{display:flex;flex-direction:column;gap:3px;min-width:125px}.assignment-load-cell b{color:var(--ink);white-space:nowrap}.assignment-load-cell small{color:var(--muted);white-space:nowrap}.assignment-load-cell.over b,.assignment-load-cell.over small{color:#b91c1c}
:root.dark-mode .assignment-load-cell.over b,:root.dark-mode .assignment-load-cell.over small{color:#fca5a5}
@media(max-width:720px){.assignment-load-card{grid-template-columns:repeat(2,minmax(0,1fr))}}
...
function teacherLoadStats(teacher, projected = null) {
  const current = Number(teacher?.assigned_periods || 0), capacity = teacher?.week_capacity == null ? null : Number(teacher.week_capacity), maxDay = Number(teacher?.max_periods_day || 0);
  const next = projected == null || !Number.isFinite(Number(projected)) ? current : Math.max(0, Number(projected));
  const remaining = capacity == null || !Number.isFinite(capacity) ? null : capacity - next;
  return { current, capacity, maxDay, projected: next, remaining, over: remaining != null && remaining < 0 };
}
function teacherLoadCardHtml(teacher, projected = null) {
  if (!teacher) return '';
  const load = teacherLoadStats(teacher, projected), capacityText = load.capacity == null ? '—' : load.capacity;
...
function teacherLoadCellHtml(teacher) {
  ...
  const load = teacherLoadStats(teacher), capacityText = load.capacity == null ? '—' : load.capacity;
...
function updateAssignmentEditLoad(form) {
  if (!form) return; const assignmentId = Number(form.querySelector('[name="assignment_id"]')?.value || 0), item = data.assignments.find(row => row.id === assignmentId), box = form.querySelector('[data-assignment-edit-load]'); if (!item || !box) return;
  const teacher = data.teachers.find(row => row.id === item.teacher_id), nextPeriods = Number(form.querySelector('[name="periods_per_week"]')?.value || 0), currentTeacherLoad = Number(teacher?.assigned_periods || 0), oldPeriods = Number(item.periods_per_week || 0), projected = Math.max(0, currentTeacherLoad - oldPeriods + nextPeriods);
  box.innerHTML = teacherLoadCardHtml(teacher, projected);
}
function updateAssignmentEditPreview(form) { updateBlockModePreview(form); updateAssignmentEditLoad(form) }
...
function assignmentTable() {
  ...
  return `<table class="data-table"><thead><tr><th>Lớp</th><th>Môn</th><th>Giáo viên</th><th>Tiết/tuần</th><th>Tải giáo viên</th><th>Chế độ xếp</th><th></th></tr></thead><tbody>${rows.map(item => { const teacher = data.teachers.find(row => row.id === item.teacher_id); return `<tr><td>${esc(item.class_name)}</td><td>${esc(item.subject_name)}</td><td>${esc(item.teacher_name)}</td><td><b>${item.periods_per_week}</b></td><td>${teacherLoadCellHtml(teacher)}</td><td>${esc(describeBlockMode(item.block_mode, item.periods_per_week))}</td><td><div class="row end"><button class="danger-link" onclick="openAssignmentEdit(${item.id})">Sửa phân công</button><button class="danger-link" onclick="delEntity('assignment',${item.id},this)">Xóa</button></div></td></tr>` }).join('')}</tbody></table>`;
}
...
function updateBulkAssignmentPreview(form) {
  const preview = $('#bulkAssignmentPreview'); if (!preview || !form) return;
  const teacherId = Number(form.querySelector('[name="teacher_id"]')?.value || 0), teacher = data.teachers.find(item => item.id === teacherId), subjectIds = [...form.querySelectorAll('input[name="subject_ids"]:checked')].map(input => Number(input.value)), classIds = [...form.querySelectorAll('input[name="class_ids"]:checked')].map(input => Number(input.value)), total = subjectIds.length * classIds.length;
  const subjectSet = new Set(subjectIds), classSet = new Set(classIds), existing = data.assignments.filter(item => subjectSet.has(item.subject_id) && classSet.has(item.class_id));
  let addedPeriods = 0; for (const subjectId of subjectIds) { const periods = Number(form.querySelector(`[name="subject_periods_${subjectId}"]`)?.value || 0); for (const classId of classIds) { if (!existing.some(item => item.subject_id === subjectId && item.class_id === classId)) addedPeriods += periods } }
  const current = Number(teacher?.assigned_periods || 0), projected = current + addedPeriods, loadBox = $('#bulkTeacherLoad'); if (loadBox) loadBox.innerHTML = teacherLoadCardHtml(teacher, projected);
  if (!total) { preview.innerHTML = 'Hãy tick ít nhất 1 môn và 1 lớp. Tải giáo viên phía trên sẽ cập nhật ngay khi bạn chọn.'; return }
  const mixedSubjects = subjectIds.filter(subjectId => bulkRequirementForSelection(subjectId, classIds).status === 'mixed'); if (mixedSubjects.length) { preview.innerHTML = `<span class="assignment-warning">Có ${mixedSubjects.length} môn có số tiết/chế độ khác nhau giữa các lớp đã chọn. Hãy chia lần phân công theo từng khối/chương trình để tránh gán sai.</span>`; return }
  const duplicates = existing.filter(item => item.teacher_id === teacherId).length, conflicts = existing.filter(item => item.teacher_id !== teacherId).length, fresh = Math.max(0, total - duplicates - conflicts), capacity = teacher?.week_capacity; let status = '';
  if (conflicts) status = `<span class="assignment-warning">Có ${conflicts} cặp lớp–môn đã thuộc giáo viên khác; hệ thống sẽ không cho lưu cho đến khi xử lý các phân công đó.</span>`; else if (capacity != null && projected > Number(capacity)) status = `<span class="assignment-warning">Tải dự kiến ${projected}/${capacity} tiết/tuần vượt khả năng xếp của giáo viên.</span>`; else status = capacity == null ? '' : `<span class="assignment-ok">Tải giáo viên: ${projected}/${capacity} tiết/tuần.</span>`;
