...
function launchConfetti() {
  ...
  function draw() {
    ...
    for (let i = 0; i < particles.length; i++) {
      ...
      p.y += ((Math.cos(p.d) + 3 + p.r / 2) / 2) * p.velocity * 0.7;
      ...
      ctx.moveTo(p.x + p.r / 2 + p.tilt, p.y);
...
async function refresh(skipOperationStatus = false) {
  ...
  const r = await fetch(`/api/projects/${PROJECT_ID}/data`, init);
  const result = await r.json().catch(() => null);
  if (!r.ok) {
    const error = new Error(
      result?.message ||
        result?.detail ||
        "Không thể tải lại dữ liệu từ máy chủ.",
    );
...
function activateWorkspaceTab(tabName) {
  ...
  if (b.dataset.tab === "constraints") renderConstraintSelectors();
  if (b.dataset.tab === "preferences") loadPreferenceInbox();
  return true;
}
document
  .querySelectorAll(".nav")
  .forEach((b) => (b.onclick = () => activateWorkspaceTab(b.dataset.tab)));
...
function renderAssignmentFilters() {
  ...
  teacherEl.innerHTML =
    '<option value="">Tất cả giáo viên</option>' + opts(data.teachers);
...
function assignmentTable() {
  ...
  if (state.status === "missing") {
    ...
    if (state.teacherId) {
      const teacher = data.teachers.find((item) => item.id === state.teacherId),
        allowed = new Set((teacher?.subject_ids || []).map(Number));
      rows = rows.filter((item) => allowed.has(item.subject_id));
    }
    if (!rows.length)
      return '<div class="assignment-filter-empty">Không có cặp lớp–môn nào lệch chương trình với bộ lọc hiện tại.</div>';
    return `<table class="data-table"><thead><tr><th>Lớp</th><th>Khối</th><th>Môn</th><th>Chương trình chuẩn</th><th>Trạng thái</th><th></th></tr></thead><tbody>${rows
      .map((item) => {
        ...
        if (item.issue_type === "mismatch") {
          ...
          return `<tr><td><b>${esc(item.class_name)}</b></td><td>${esc(item.grade_name)}</td><td>${esc(item.subject_name)}</td><td>${esc(required)}</td><td><span class="assignment-gap-badge">Đang có: ${esc(current)}</span></td><td><button class="danger-link" onclick="openAssignmentEdit(${item.assignment_id})">Sửa phân công</button></td></tr>`;
        }
  ...
  if (state.status === "unassigned_class") {
    ...
    return `<table class="data-table"><thead><tr><th>Lớp</th><th>Khối</th><th>Trạng thái</th><th></th></tr></thead><tbody>${rows
      .map((item) => {
        ...
        return `<tr><td><b>${esc(item.name)}</b></td><td>${esc(grade?.name || "—")}</td><td><span class="assignment-gap-badge">Chưa có phân công</span></td><td><button class="danger-link" onclick="openEntity('assignment')">+ Thêm phân công</button></td></tr>`;
      })
      .join("")}</tbody></table>`;
  }
...
function openEntity(type, preset = null) {
  ...
  if (type === "subject")
    h +=
      '<label>Tên rút gọn<input name="short_name" required></label><label>Số tiết liên tiếp tối đa<input type="number" name="max_consecutive" value="2" min="1" max="4"></label><p class="muted">Giới hạn số tiết của môn này được phép xếp liền nhau trong cùng một buổi.</p>';
  if (type === "teacher")
    h = `<label>Họ tên<input name="name" required></label><label>Tên ngắn<input name="short_name" required></label><label>Tổ chuyên môn<select name="department_id"><option value="">Không chọn</option>${opts(data.departments)}</select></label><label>Tối đa tiết/ngày<input type="number" name="max_periods_day" value="5" min="1" max="10"></label>${teacherSubjectPicker()}`;
  if (type === "grade")
  ...
  if (type === "assignment")
    h = `<div class="assignment-bulk-modal"><p class="assignment-bulk-intro">Chọn giáo viên trước, tick môn và lớp. Nếu các lớp đã có chương trình chuẩn theo khối, số tiết/tuần và chế độ xếp sẽ tự điền theo chương trình; hệ thống không lấy lại số tiết từ một lớp khác của giáo viên.</p><label>Giáo viên<select name="teacher_id" required onchange="renderBulkAssignmentChoices(this.value)"><option value="">Chọn giáo viên</option>${opts(data.teachers)}</select></label><div id="bulkAssignmentChoices"><div class="assignment-bulk-empty">Chọn giáo viên để hiện danh sách môn và lớp.</div></div></div>`;
...
function updateBulkAssignmentPreview(form) {
  ...
  preview.innerHTML = `Đã chọn <b>${subjectIds.length}</b> môn × <b>${classIds.length}</b> lớp = <b>${total}</b> cặp. ${duplicates ? `Bỏ qua ${duplicates} cặp đã có; ` : ""}dự kiến tạo mới <b>${fresh}</b> phân công, thêm <b>${addedPeriods}</b> tiết/tuần.${status}`;
}
  ...
  try {
    if (entityType === "assignment") {
      ...
      if (!r.ok) {
        entityActionError(
          submitButton,
          result.message || result.detail || "Không thể thêm phân công.",
        );
        return;
      }
    ...
    if (entityType === "assignment_edit") {
      ...
      if (!r.ok) {
        entityActionError(
          submitButton,
          result.message || result.detail || "Không thể cập nhật phân công.",
        );
        return;
      }
      setInlineActionState(submitButton, "success", {
    ...
    if (entityType.endsWith("_edit")) {
      ...
      if (!r.ok) {
        entityActionError(
          submitButton,
          result.message || result.detail || "Không thể cập nhật dữ liệu.",
        );
        return;
      }
      setInlineActionState(submitButton, "success", {
      ...
      setEntityActionMessage("Đã cập nhật dữ liệu.", "success");
      await wait(650);
      entityModal.close();
      await refresh(true);
      return;
    ...
    if (!r.ok) {
      entityActionError(
        submitButton,
        result.message || result.detail || "Không thể lưu dữ liệu.",
      );
      return;
    }
    setInlineActionState(submitButton, "success", {
...
async function delEntity(type, id, button) {
  ...
  try {
    ...
    setInlineActionState(
      button,
      "error",
      { idle: "Xóa", error: "Lỗi kết nối" },
      2200,
    );
    showInlineActionFeedback(button, "Mất kết nối tới máy chủ.", "error", 5000);
  }
}
function handleExportExcel(event, link) {
  if (!link) return;
  const originalHtml = link.innerHTML;
  link.innerHTML = '<span class="btn-spinner"></span> Đang tạo tệp...';
  link.style.pointerEvents = "none";
  showToast(
    "Đang chuẩn bị file Excel, tệp sẽ được tải xuống tự động...",
    "info",
    3500,
  );
  setTimeout(() => {
...
function computeScheduleConflicts() {
  ...
  if (!data || !data.lessons || !data.assignments)
    return { conflictsByLessonId, conflictsBySlot };

  const slotMap = new Map();
  for (const l of data.lessons) {
    if (!slotMap.has(l.slot)) slotMap.set(l.slot, []);
    slotMap.get(l.slot).push(l);
  }

  const blockedSlots = new Set(data.project?.blocked_slots || []);

  for (const [slot, lessons] of slotMap.entries()) {
    ...
    for (const l of lessons) {
      ...
      if (!teacherMap.has(a.teacher_id)) teacherMap.set(a.teacher_id, []);
      teacherMap.get(a.teacher_id).push({ lesson: l, assignment: a });

      if (!classMap.has(a.class_id)) classMap.set(a.class_id, []);
      classMap.get(a.class_id).push({ lesson: l, assignment: a });
    }
    ...
    for (const [teacherId, list] of teacherMap.entries()) {
      if (list.length > 1) {
        const teacherName =
          list[0].assignment.teacher_name ||
          list[0].assignment.teacher_short ||
          "Giáo viên";
        const classNames = list
          .map((item) => item.assignment.class_name)
          .join(", ");
        const reason = `Trùng lịch GV: ${teacherName} đang dạy cùng lúc ở các lớp (${classNames})`;
        for (const item of list) {
          conflictsByLessonId.set(item.lesson.id, reason);
        }
        if (!conflictsBySlot.has(slot)) conflictsBySlot.set(slot, []);
        conflictsBySlot.get(slot).push(reason);
      }
    }

    // Trùng lịch lớp
    for (const [classId, list] of classMap.entries()) {
      if (list.length > 1) {
        const className = list[0].assignment.class_name || "Lớp";
        const subs = list
          .map(
            (item) =>
              item.assignment.subject_short || item.assignment.subject_name,
          )
          .join(", ");
        const reason = `Trùng lịch lớp: ${className} có nhiều môn (${subs}) trong cùng 1 tiết`;
        for (const item of list) {
          conflictsByLessonId.set(item.lesson.id, reason);
        }
        if (!conflictsBySlot.has(slot)) conflictsBySlot.set(slot, []);
    ...
    // Tiết bị khóa toàn trường
    if (blockedSlots.has(slot)) {
      const reason = "Tiết này đã bị khóa toàn trường";
      for (const l of lessons) {
        conflictsByLessonId.set(l.id, reason);
      }
      if (!conflictsBySlot.has(slot)) conflictsBySlot.set(slot, []);
      conflictsBySlot.get(slot).push(reason);
    }
    ...
    if (r.ok) {
      ...
      setTimeout(async () => {
        ...
        try {
          ...
          setScheduleActionState("error", 2200);
          setScheduleFeedback(
            "Máy chủ đã xếp lịch nhưng không thể tải lại dữ liệu mới. Hãy tải lại trang để đồng bộ thời khóa biểu.",
            "error",
            6500,
          );
          return;
        ...
        renderSchedule();
        setScheduleActionState("success", 1400);
        setScheduleFeedback(
      ...
      setScheduleFeedback(
        j.message ||
          "Xếp thời khóa biểu thất bại; lịch hiện tại được giữ nguyên.",
        "error",
        5200,
      );
      showToast(j.message || "Xếp thời khóa biểu thất bại.", "error", 4500);
    }
  } catch {
    clearTimeout(stepTimer1);
    clearTimeout(stepTimer2);
...
function clusteredLessonIds() {
  ...
  const eligible = new Set(
    data.assignments
      .filter(
        (item) =>
          item.block_mode === "preferred_double" ||
          item.block_mode === "required_double",
      )
      .map((item) => item.id),
  );
  ...
  for (const lesson of data.lessons) {
    if (!eligible.has(lesson.assignment_id)) continue;
...
function renderSchedule(preserveScroll = false) {
  ...
  const legend =
    '<div class="schedule-color-legend"><b>Chú thích màu</b><span><i class="schedule-color-swatch"></i>Tiết đơn</span><span><i class="schedule-color-swatch cluster"></i>Tiết đang được ghép đôi</span><span><i class="schedule-color-swatch" style="background:#fee2e2;border:1px dashed #ef4444"></i>Ô xung đột trùng tiết</span></div>';

  const filterLesson = (l) => {
    const a = data.assignments.find((x) => x.id === l.assignment_id);
    if (!a) return false;
    if (vt.value === "overview") return true;
    return vt.value === "class"
      ? a.class_id === Number(ve.value)
      : vt.value === "subject"
        ? a.subject_id === Number(ve.value)
        : a.teacher_id === Number(ve.value);
  };

  const sortLessons = (left, right) => {
    const a = data.assignments.find((x) => x.id === left.assignment_id);
    const b = data.assignments.find((x) => x.id === right.assignment_id);
...
function lessonHtml(l, view, inCluster = false, conflictReason = null) {
  const a = data.assignments.find((x) => x.id === l.assignment_id);
  if (!a) return "";
  const detail = `${a.class_name} · ${a.teacher_short}`;
  const actions =
    window.READ_ONLY || l._syncing
      ? ""
      : l.locked
        ? `<button class="lesson-remove" title="Bỏ cố định tiết/cặp này" onclick="event.stopPropagation();unfixGroup(${a.id},${l.slot},this)">🔓</button>`
        : `<button class="lesson-pin" title="Cố định tiết/cặp này" onclick="event.stopPropagation();setFixed(${a.id},${l.slot},this)">📌</button><button class="lesson-remove" title="Gỡ tiết" onclick="event.stopPropagation();removeLesson(${l.id})">×</button>`;
  const dragPayload =
    !window.READ_ONLY && !l.locked && !l._syncing
      ? ` data-drag-payload="${l.id}"`
      : "";
  const syncing = l._syncing
    ? `<span class="lesson-sync-indicator" role="status" aria-live="polite" title="Đang lưu thay đổi"><span class="lesson-sync-spinner" aria-hidden="true"></span><span class="lesson-sync-text">Đang lưu</span></span>`
    : "";
  const conflictBadge = conflictReason
    ? `<span class="conflict-badge" title="${esc(conflictReason)}" onclick="event.stopPropagation();showToast('${esc(conflictReason)}', 'warning')">!</span>`
    : "";
  return `<div class="lesson ${view === "overview" ? "lesson-overview" : ""} ${inCluster ? "lesson-cluster" : ""} ${l._syncing ? "lesson-syncing" : ""} ${conflictReason ? "has-conflict" : ""}" draggable="false"${dragPayload} title="${conflictReason ? esc(conflictReason) : ""}"><b>${esc(a.subject_short)}</b><small>${esc(detail)}</small>${l.locked ? ' <span title="Tiết cố định">🔒</span>' : ""}${conflictBadge}${syncing}${actions}</div>`;
}
...
async function dropLessonPayload(raw, slot) {
  if (window.READ_ONLY || !raw || pendingDropPayloads.has(raw)) return;
  ...
  let optimisticLesson = null,
    oldSlot = null;
  if (isNew) {
    const assignment = data.assignments.find((item) => item.id === id);
    if (!assignment) return;
    optimisticLesson = {
      id: optimisticLessonId--,
      assignment_id: id,
      slot,
      locked: false,
      _syncing: true,
    };
    data.lessons.push(optimisticLesson);
    renderOptimisticSchedule();
  } else {
    optimisticLesson = data.lessons.find((item) => item.id === id);
    if (
      !optimisticLesson ||
      optimisticLesson.locked ||
      optimisticLesson._syncing
    )
      return;
    if (optimisticLesson.slot === slot) return;
    oldSlot = optimisticLesson.slot;
    optimisticLesson.slot = slot;
    optimisticLesson._syncing = true;
    renderOptimisticSchedule();
  }
  ...
  try {
    ...
    if (r.ok) {
      ...
      if (isNew) {
        const live = data.lessons.find(
          (item) => item.id === optimisticLesson.id,
        );
        if (live) {
          live.id = Number(j.id) || live.id;
          delete live._syncing;
        } else stateWasReplaced = true;
      } else {
        const live = data.lessons.find((item) => item.id === id);
        if (live && live._syncing) {
          live.slot = slot;
          delete live._syncing;
        } else stateWasReplaced = true;
      }
    ...
    renderOptimisticSchedule();
    showDropConflict(slot, j.message || j.detail || "Không thể xếp tiết");
  } catch {
    if (isNew)
      data.lessons = data.lessons.filter(
        (item) => item.id !== optimisticLesson.id,
    ...
    renderOptimisticSchedule();
    showDropConflict(slot, "Mất kết nối tới máy chủ");
  } finally {
    pendingDropPayloads.delete(raw);
  }
}
async function dropLesson(e, slot) {
  return dropLessonPayload(e?.dataTransfer?.getData("text/plain") || "", slot);
}
async function removeLesson(id) {
  ...
  try {
    ...
    } else setTrayActionStatus("error", j.message || "Không thể gỡ tiết", 3600);
  } catch {
    setTrayActionStatus(
      "error",
...
async function setFixed(assignmentId, slot, button) {
  ...
  try {
    ...
    if (r.ok) {
      ...
      button.textContent = "!";
      button.title = j.message || j.detail || "Không thể cố định tiết";
      setTimeout(() => {
    ...
    if (button) {
      button.disabled = false;
      button.textContent = "!";
      button.title = "Mất kết nối tới máy chủ";
    }
  }
}
async function unfixGroup(assignmentId, slot, button) {
  ...
  try {
    ...
    if (r.ok) {
      ...
      button.disabled = false;
      button.textContent = "!";
      button.title = j.message || j.detail || "Không thể bỏ cố định";
      setTimeout(() => {
        if (button.isConnected) {
          button.textContent = original;
          button.disabled = false;
...
async function returnAssignmentToTray(id) {
  ...
  try {
    ...
    if (r.ok) {
      ...
      setTrayActionStatus(
        "success",
        j.message || "Đã đưa phân công về khay",
        1800,
      );
    } else
      setTrayActionStatus(
        "error",
        j.message || "Không thể đưa phân công về khay",
        3600,
      );
  } catch {
    setTrayActionStatus(
...
async function returnAllToTray(button) {
  ...
  try {
    ...
    setInlineActionState(
      button,
      "error",
      { idle: "Đưa tiết chưa cố định về khay", error: "Lỗi kết nối" },
      2200,
    );
    showInlineActionFeedback(button, "Mất kết nối tới máy chủ.", "error", 5000);
  }
}
async function dropToTrayPayload(raw) {
  if (window.READ_ONLY || !raw || raw.startsWith("assignment:")) return;
  if (raw.startsWith("scheduled-assignment:")) {
    await returnAssignmentToTray(Number(raw.split(":")[1]));
    return;
  }
  if (/^\d+$/.test(raw)) await removeLesson(Number(raw));
}
async function dropToTray(e) {
  return dropToTrayPayload(e?.dataTransfer?.getData("text/plain") || "");
}
...
async function saveGlobalSessionLocks(button) {
  ...
  try {
    const r = await fetch(`/api/projects/${PROJECT_ID}/session-locks`, {
      method: "POST",
      headers: operationHeaders({ "Content-Type": "application/json" }),
      body: JSON.stringify({ sessions, slots }),
    });
    const result = await r.json();
    if (r.ok) {
      await refresh(true);
    ...
    setInlineActionState(
      button,
      "error",
      { idle: "Lưu khóa lịch", error: "Lỗi kết nối" },
      2200,
    );
    showInlineActionFeedback(button, "Mất kết nối tới máy chủ.", "error", 5000);
  }
...
function renderConstraintSelectors() {
  ...
  ent.onchange = renderConstraintGrid;
  renderConstraintGrid();
}
...
async function saveConstraints(button) {
  ...
  try {
    const r = await fetch(`/api/projects/${PROJECT_ID}/constraints`, {
      method: "POST",
      headers: operationHeaders({ "Content-Type": "application/json" }),
      body: JSON.stringify({
    ...
    setInlineActionState(
      button,
      "error",
      { idle: "Lưu ràng buộc", error: "Lỗi kết nối" },
      2200,
    );
    showInlineActionFeedback(button, "Mất kết nối tới máy chủ.", "error", 5000);
  }
...
async function loadPreferenceInbox() {
  ...
  const r = await fetch(`/api/projects/${PROJECT_ID}/preferences`);
  if (!r.ok) {
    box.innerHTML =
      '<div class="empty-state">Không thể tải nguyện vọng giáo viên.</div>';
    return;
  }
  const result = await r.json();
...
async function reviewPreference(id, action, button) {
  ...
  try {
    const r = await fetch(
      `/api/projects/${PROJECT_ID}/preferences/${id}/review`,
      {
        method: "POST",
        headers: operationHeaders({ "Content-Type": "application/json" }),
    ...
    setInlineActionState(button, "error", { idle, error: "Lỗi kết nối" }, 2200);
    showInlineActionFeedback(button, "Mất kết nối tới máy chủ.", "error", 5000);
  }
}
async function copyShare(button) {
  try {
    await navigator.clipboard.writeText(window.SHARE_URL);
    setInlineActionState(
      button,
      "success",
    ...
    setInlineActionState(
      button,
      "error",
      { idle: "Chia sẻ", error: "Không sao chép được" },
      2200,
    );
  }
}
function markGlobalBlockedSlots() {
  ...
  for (const slot of blocked) {
    ...
    document.querySelectorAll(`[data-slot="${slot}"]`).forEach((cell) => {
      ...
      cell.title = wholeSession ? "Buổi này đã bị khóa" : "Tiết này đã bị khóa";
      if (!cell.querySelector(".global-lock-label"))
...
function stopDragAutoScroll() {
  if (dragAutoScroll.frame) {
    cancelAnimationFrame(dragAutoScroll.frame);
    dragAutoScroll.frame = 0;
  }
}
function dragScrollableAncestorAt(
  clientX = pointerDrag.lastX,
  clientY = pointerDrag.lastY,
  axis = "y",
) {
...
function scrollDragViewport(top = 0, left = 0) {
  let moved = false;
  if (top) {
    const vertical = dragScrollableAncestorAt(
      pointerDrag.lastX,
      pointerDrag.lastY,
      "y",
    );
  ...
  if (left) {
    const horizontal = dragScrollableAncestorAt(
      pointerDrag.lastX,
      pointerDrag.lastY,
      "x",
    );
...
function dragEdgeScrollSpeed() {
  ...
  const edge = Math.max(90, Math.min(150, viewportHeight * 0.16)),
    y = pointerDrag.lastY;
  if (y < edge) {
    const strength = Math.max(0, Math.min(1, (edge - y) / edge));
    return -(3 + 17 * strength * strength) * DRAG_EDGE_SCROLL_MULTIPLIER;
  }
  if (y > viewportHeight - edge) {
    const strength = Math.max(
      0,
      Math.min(1, (y - (viewportHeight - edge)) / edge),
    );
    return (3 + 17 * strength * strength) * DRAG_EDGE_SCROLL_MULTIPLIER;
  }
  return 0;
...
function clearPointerDropTarget() {
  if (pointerDrag.target) pointerDrag.target.classList.remove("drag-over");
...
function canDropPointerPayload(payload, target) {
  ...
  if (target.classList.contains("unscheduled-tray"))
    return !payload.startsWith("assignment:");
  if (
    target.classList.contains("cell") &&
    target.classList.contains("available")
  )
    return !payload.startsWith("scheduled-assignment:");
  return false;
}
function pointerDropTargetAt(clientX, clientY) {
  const hit = document.elementFromPoint(clientX, clientY),
    target = hit?.closest?.(".cell.available,.unscheduled-tray") || null;
...
function updatePointerDropTarget(
  ...
  const next = pointerDropTargetAt(clientX, clientY);
  if (next === pointerDrag.target) return;
...
function startDragAutoScroll() {
  if (pointerDrag.active && !dragAutoScroll.frame)
    dragAutoScroll.frame = requestAnimationFrame(runDragAutoScroll);
}
function updateDragAutoScroll(clientY) {
  if (Number.isFinite(clientY)) pointerDrag.lastY = clientY;
  startDragAutoScroll();
}
function normalizedWheelDelta(value, mode) {
  if (mode === 1) return value * 18;
  if (mode === 2) return value * (window.innerHeight || 600);
  return value;
}
function makePointerDragGhost(source, event) {
  ...
  ghost.removeAttribute("data-drag-payload");
  ...
  Object.assign(ghost.style, {
    ...
    boxShadow: "0 18px 42px rgba(15,23,42,.26)",
    transition: "none",
  });
...
function beginPointerDrag(event, source) {
  ...
  pointerDrag.source = source;
  pointerDrag.payload = source.dataset.dragPayload || "";
  pointerDrag.lastX = pointerDrag.startX = event.clientX;
  pointerDrag.lastY = pointerDrag.startY = event.clientY;
  pointerDrag.didMove = false;
  pointerDrag.oldUserSelect = document.body.style.userSelect;
  document.body.style.userSelect = "none";
  source.classList.add("dragging");
  document.body.classList.add("is-dragging");
  makePointerDragGhost(source, event);
...
function clearPointerDrag() {
  ...
  document.body.style.userSelect = pointerDrag.oldUserSelect;
  pointerDrag.active = false;
  pointerDrag.pointerId = null;
  pointerDrag.source = null;
  pointerDrag.payload = "";
...
function finishPointerDrag(event, cancel = false) {
  if (
    !pointerDrag.active ||
    (event?.pointerId != null && pointerDrag.pointerId !== event.pointerId)
  )
    return;
  ...
  if (Number.isInteger(slot)) dropLessonPayload(payload, slot);
}
...
document.addEventListener(
  "pointerup",
  (event) => finishPointerDrag(event, false),
  { capture: true },
);
document.addEventListener(
  "pointercancel",
  (event) => finishPointerDrag(event, true),
  { capture: true },
);
...
window.addEventListener("wheel", handlePointerDragWheel, {
  capture: true,
  passive: false,
});
