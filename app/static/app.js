...
let data=window.INIT_DATA;let entityType='';let entityId=null;let teacherAccounts=[];let pendingAssignmentGap=null;const $=s=>document.querySelector(s);const esc=s=>String(s??'').replace(/[&<>'"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));
...
async function refresh(skipOperationStatus=false){const init=skipOperationStatus?{headers:operationHeaders()}:undefined;const r=await fetch(`/api/projects/${PROJECT_ID}/data`,init);if(r.ok)data=await r.json();renderAll()}
...
async function submitEntity(e){
  ...
  try{
    ...
    if(entityType==='teacher_account'){
      const r=await fetch(`/api/projects/${PROJECT_ID}/teacher-accounts`,{method:'POST',headers:operationHeaders({'Content-Type':'application/json'}),body:JSON.stringify(o)});const result=await r.json();
      if(!r.ok){entityActionError(submitButton,result.detail||'Không thể cấp tài khoản.');return}
      setInlineActionState(submitButton,'success',{idle:'Lưu',success:'Đã lưu'});setEntityActionMessage('Đã lưu tài khoản giáo viên.','success');await wait(650);entityModal.close();await loadTeacherAccounts();return;
    }
...
async function delEntity(type,id,button){
  ...
  try{const r=await fetch(`/api/projects/${PROJECT_ID}/entity/${type}/${id}`,{method:'DELETE',headers:operationHeaders()});
    if(r.ok){setInlineActionState(button,'success',{idle:'Xóa',success:'Đã xóa'});await wait(450);await refresh(true)}
    else{setInlineActionState(button,'error',{idle:'Xóa',error:'Không thể xóa'},2200);showInlineActionFeedback(button,'Dữ liệu này đang được sử dụng nên chưa thể xóa.','error',5000)}
...
async function generateSchedule(allowRebuild=false){
  ...
  try{
    ...
    if(r.ok){await refresh(true);renderSchedule();setScheduleActionState('success',1400);setScheduleFeedback(j.message||'Đã xếp thời khóa biểu thành công.','success',2600);launchConfetti()}
...
async function returnAllToTray(button){
  if(!await confirmAction('Đưa toàn bộ tiết đang xếp về khay? Phân công và số tiết/tuần vẫn được giữ nguyên.',{title:'Đưa lịch về khay',confirmText:'Đưa về khay'}))return;
  setInlineActionState(button,'loading',{idle:'Đưa toàn bộ về khay',loading:'Đang đưa về khay...'});
...
async function saveGlobalSessionLocks(button){
  ...
  try{const r=await fetch(`/api/projects/${PROJECT_ID}/session-locks`,{method:'POST',headers:operationHeaders({'Content-Type':'application/json'}),body:JSON.stringify({sessions,slots})});const result=await r.json();
...
async function saveConstraints(button){
  ...
  try{const r=await fetch(`/api/projects/${PROJECT_ID}/constraints`,{method:'POST',headers:operationHeaders({'Content-Type':'application/json'}),body:JSON.stringify({entity_type:$('#constraintType').value,entity_id:Number(entity.value),slots})});const result=await r.json();
...
async function loadPreferenceInbox(){const box=$('#preferenceInbox');if(!box)return;box.innerHTML='<div class="empty-state">Đang tải hòm thư…</div>';const r=await fetch(`/api/projects/${PROJECT_ID}/preferences`);if(!r.ok){box.innerHTML='<div class="empty-state">Không thể tải hòm thư.</div>';return}const result=await r.json();if(!result.items.length){box.innerHTML='<div class="empty-state">Chưa có nguyện vọng nào được gửi.</div>';return}const statuses={pending:'Chờ duyệt',accepted:'Đang áp dụng',rejected:'Đã từ chối',superseded:'Đã thay thế'};box.innerHTML=`<div class="preference-list">${result.items.map(item=>`<article class="preference-card"><div class="preference-card-head"><div><h3>${esc(item.teacher_name)}</h3><small>${esc(item.created_at.replace('T',' '))}</small></div><span class="status status-${item.status}">${statuses[item.status]||esc(item.status)}</span></div><div class="preference-detail"><b>Tiết mong muốn</b><div class="slot-chips">${item.preferred_slots.length?item.preferred_slots.map(slot=>`<span class="slot-chip preferred">${preferenceSlotLabel(slot)}</span>`).join(''):'<span class="muted">Không đăng ký</span>'}</div></div><div class="preference-detail"><b>Tiết cần tránh</b><div class="slot-chips">${item.unavailable_slots.length?item.unavailable_slots.map(slot=>`<span class="slot-chip blocked">${preferenceSlotLabel(slot)}</span>`).join(''):'<span class="muted">Không đăng ký</span>'}</div></div>${item.note?`<p class="preference-note">${esc(item.note)}</p>`:''}${item.status==='pending'?`<div class="row end"><button class="btn ghost" onclick="reviewPreference(${item.id},'reject',this)">Từ chối</button><button class="btn" onclick="reviewPreference(${item.id},'accept',this)">Chấp nhận & áp dụng</button></div>`:''}</article>`).join('')}</div>`}
async function reviewPreference(id,action,button){
  const idle=action==='accept'?'Chấp nhận & áp dụng':'Từ chối',loading=action==='accept'?'Đang áp dụng...':'Đang từ chối...',success=action==='accept'?'Đã áp dụng':'Đã từ chối';setInlineActionState(button,'loading',{idle,loading});
  try{const r=await fetch(`/api/projects/${PROJECT_ID}/preferences/${id}/review`,{method:'POST',headers:operationHeaders({'Content-Type':'application/json'}),body:JSON.stringify({action})});const result=await r.json();
...
async function loadTeacherAccounts(){const box=$('#teacherAccountTable');if(!box)return;const r=await fetch(`/api/projects/${PROJECT_ID}/teacher-accounts`);if(!r.ok){box.innerHTML='<div class="empty-state">Không thể tải tài khoản.</div>';return}teacherAccounts=(await r.json()).items;box.innerHTML=`<table class="data-table"><thead><tr><th>Giáo viên</th><th>Vai trò</th><th>Email đăng nhập</th><th></th></tr></thead><tbody>${teacherAccounts.map(item=>`<tr><td>${esc(item.teacher_name)}</td><td><span class="status ${item.account_id?'status-accepted':''}">${item.account_id?'Giáo viên bộ môn':'Chưa cấp quyền'}</span></td><td>${esc(item.email||'—')}</td><td><div class="row end"><button class="danger-link" onclick="openTeacherAccount(${item.teacher_id},this)">${item.account_id?'Cập nhật':'Cấp tài khoản'}</button>${item.account_id?`<button class="danger-link" onclick="revokeTeacherAccount(${item.teacher_id},this)">Thu hồi</button>`:''}</div></td></tr>`).join('')}</tbody></table>`}
function openTeacherAccount(teacherId,button){entityType='teacher_account';const selectedId=Number(teacherId),current=teacherAccounts.find(item=>item.teacher_id===selectedId),isExisting=Boolean(current?.account_id);let teacherField;if(isExisting){teacherField=`<input type="hidden" name="teacher_id" value="${current.teacher_id}"><label>Giáo viên<select disabled><option>${esc(current.teacher_name)}</option></select></label>`}else{const available=teacherAccounts.filter(item=>!item.account_id);if(!available.length){setInlineActionState(button,'error',{error:'Đã cấp hết'},1800);showInlineActionFeedback(button,'Tất cả giáo viên đã có tài khoản.','info',3600);return}teacherField=`<label>Giáo viên<select name="teacher_id" required><option value="">Chọn giáo viên</option>${available.map(item=>`<option value="${item.teacher_id}" ${item.teacher_id===selectedId?'selected':''}>${esc(item.teacher_name)}</option>`).join('')}</select></label>`}$('#modalTitle').textContent=isExisting?'Cập nhật tài khoản giáo viên':'Cấp tài khoản giáo viên';$('#entityFields').innerHTML=`${teacherField}<label>Email đăng nhập<input type="email" name="email" value="${esc(current?.email||'')}" required></label><label>${isExisting?'Mật khẩu mới (để trống nếu giữ nguyên)':'Mật khẩu'}<input type="password" name="password" minlength="8" ${isExisting?'':'required'}></label><p class="muted">Quyền giáo viên chỉ cho phép xem lịch cá nhân và gửi nguyện vọng.</p>`;entityModal.showModal()}
async function revokeTeacherAccount(teacherId,button){
  if(!await confirmAction('Thu hồi quyền đăng nhập của giáo viên này?',{title:'Thu hồi tài khoản',confirmText:'Thu hồi'}))return;
  setInlineActionState(button,'loading',{idle:'Thu hồi',loading:'Đang thu hồi...'});
  try{const r=await fetch(`/api/projects/${PROJECT_ID}/teacher-accounts/${teacherId}`,{method:'DELETE',headers:operationHeaders()});if(r.ok){setInlineActionState(button,'success',{idle:'Thu hồi',success:'Đã thu hồi'});await wait(500);await loadTeacherAccounts()}else{setInlineActionState(button,'error',{idle:'Thu hồi',error:'Chưa thu hồi'},2200);showInlineActionFeedback(button,'Không thể thu hồi tài khoản.','error',5000)}}
