(() => {
  ...
  document.addEventListener("pointermove", (event) => {
    if (
      card.classList.contains("panda-shy") ||
      document.activeElement === email
    )
      return;
    const face = panda.getBoundingClientRect();
    setEyes(
      (event.clientX - face.left - face.width / 2) / 32,
      (event.clientY - face.top - face.height / 2) / 35,
    );
  });

  email.addEventListener("focus", () => card.classList.remove("panda-shy"));
  email.addEventListener("input", () =>
    setEyes(-3.5 + Math.min(email.value.length, 28) / 4, 2),
  );
  ...
  if (toggle) {
    const setPasswordVisible = (visible) => {
      ...
      toggle.setAttribute(
        "aria-label",
        visible ? "Bấm để ẩn mật khẩu" : "Bấm để hiện mật khẩu",
      );
    };
