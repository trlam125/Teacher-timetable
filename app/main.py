...
class Project(Base):
    ...
    created_at: Mapped[str] = mapped_column(
        String(40), default=lambda: datetime.now().isoformat(timespec="seconds")
    )
...
def required_double_hard_feasible(
    ...
    *,
    ...
    thực sự có thể đặt vào các ô còn dùng được, không vượt giới hạn tiết/ngày
    và các cụm trong cùng buổi không dính liền thành cụm dài hơn.
    """
    ...
    def session_options(day: int, session: int):
        ...
        def walk(period: int):
            ...
            if period in memo:
                return memo[period]
            options = set(walk(period + 1))
            if allowed[period]:
                for doubles, singles in walk(period + 2):
                    if singles < 1:
                        options.add((doubles, singles + 1))
            if period + 1 < pps and allowed[period] and allowed[period + 1]:
                for doubles, singles in walk(period + 3):
    ...
    *,
    ...
    if not required_double_hard_feasible(
        ...
        raise HTTPException(
            ...
            f"với {teacher.name} theo tiết tránh, buổi/tiết khóa và giới hạn tiết/ngày hiện tại.",
        )
...
def _complete_pattern_placement(
    ...
    def compatible(left_start: int, left_size: int, right_start: int, right_size: int):
        ...
        return (
            ...
            and right_start + right_size != left_start
        )
...
async def auth_error(request: Request, exc):
    if request.url.path.startswith("/api/"):
        return JSONResponse(
            {"ok": False, "message": "Phiên đăng nhập đã hết hạn."}, status_code=401
        )
...
def admin_users(
    ...
    users = [
        account for account in all_users if admin_can_manage_account(user, account)
    ]
    managed_account_ids = {
        account.id
...
def update_account(
    ...
    if password_changed:
        if len(password.strip()) < MIN_PASSWORD_LENGTH:
            raise HTTPException(
                400, f"Mật khẩu mới phải có ít nhất {MIN_PASSWORD_LENGTH} ký tự"
            )
...
def create_project(
    ...
    if not clean_school_name:
        raise HTTPException(400, "Tên trường không được để trống")
...
def add_entity(
    ...
    if payload.type == "department":
        ...
        max_consecutive = bounded_int(
            d.get("max_consecutive"), 2, 1, 4, "Số tiết liên tiếp tối đa"
        )
...
def constraints(
    ...
    for assignment in assignments:
        ...
        if not assignment_requires_double(assignment):
            ...
            continue
        for run in assignment_run_groups(
...
def fixed(
    ...
    if assignment_requires_double(assignment):
        run = next(
            (
                item
                for item in assignment_run_groups(
...
def return_assignment_to_tray(
    ...
    removable = [lesson for lesson in lessons if not lesson.locked]
    locked = len(lessons) - len(removable)
    for lesson in removable:
        db.delete(lesson)
    db.commit()
    message = f"Đã đưa {len(removable)} tiết về khay."
    if locked:
        message += f" Còn {locked} tiết cố định được giữ lại."
...
def verify_email_change(
    ...
    if not target or (
        target.id != user.id and not admin_can_manage_account(user, target)
    ):
        raise HTTPException(403, "Bạn không còn quyền đổi email tài khoản này")
    now = datetime.now(timezone.utc)
...
def ga_schedule(
    ...
    def cp_sat_primary():
        ...
        for index, task in enumerate(task_rows):
            ...
            for slot in sorted(set(candidate_pool)):
                ...
                if explicit:
                    ...
                    right = (
                        slot + size if period + size < p.periods_per_session else None
                    )
                    if (
                        left is not None
                        and left in existing_assignment_slots[assignment.id]
                    ) or (
                        right is not None
                        and right in existing_assignment_slots[assignment.id]
                    ):
                        continue
        ...
        for assignment_id, rows in assignment_option_rows.items():
            assignment = next(
                (row for row in assignments if row.id == assignment_id), None
            )
            ...
            for i, left in enumerate(rows):
                ...
                for right in rows[i + 1 :]:
                    ...
                    if li == ri:
                        continue
                    same_session = (lstart // p.periods_per_session) == (
                        rstart // p.periods_per_session
                    )
        ...
        for index, task in enumerate(task_rows):
            ...
            for slot in candidate_pool:
                ...
                if explicit:
                    ...
                    if any(
                        neighbor in assignment_busy[assignment.id]
